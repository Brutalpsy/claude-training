...
// Mock ToolCallLabel to isolate MessageList tests
vi.mock("../ToolCallLabel", () => ({
  ToolCallLabel: ({ toolInvocation }: any) => (
    <div data-testid="tool-call-label">{toolInvocation.toolName}:{toolInvocation.args?.command}</div>
  ),
}));
...
test("MessageList shows empty state when no messages", () => {
  ...
  expect(
    screen.getByText("Chat with an LLM to generate React components")
  ).toBeDefined();
  expect(
...
test("MessageList renders messages with parts", () => {
  ...
  expect(screen.getByTestId("tool-call-label")).toBeDefined();
});
